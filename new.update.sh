...
# 确保目录存在
mkdir -p dists/LF/main/binary-all
mkdir -p dists/LF/main/binary-iphoneos-arm
mkdir -p dists/LF/main/binary-iphoneos-arm64
mkdir -p dists/LF/main/binary-iphoneos-arm64e
# mkdir -p dists/LF/dev/binary-all
# mkdir -p dists/LF/dev/binary-iphoneos-arm
# mkdir -p dists/LF/dev/binary-iphoneos-arm64
# mkdir -p dists/LF/dev/binary-iphoneos-arm64e
...
ARCH_POOL_MAP["all"]="pool/main/all"
ARCH_POOL_MAP["iphoneos-arm"]="pool/main/arm"
ARCH_POOL_MAP["iphoneos-arm64"]="pool/main/arm64"
ARCH_POOL_MAP["iphoneos-arm64e"]="pool/main/arm64e"
...
for component in "${COMPONENTS[@]}"; do
    for arch in all iphoneos-arm iphoneos-arm64 iphoneos-arm64e; do
        output_dir="dists/LF/${component}/binary-${arch}"
...
for component in "${COMPONENTS[@]}"; do
    has_packages=false
    for arch in all iphoneos-arm iphoneos-arm64 iphoneos-arm64e; do
        if [ -s "dists/LF/${component}/binary-${arch}/Packages" ]; then
            has_packages=true
            break
        fi
    done
...
for component in $components_list; do
    for arch in $arches_list; do
        binary_dir="dists/LF/${component}/binary-${arch}"
...
# 创建根目录的 Release 符号链接
ln -sf dists/LF/Release Release 2>/dev/null || true
echo "[5/5] 提交..."
# ============================================
# 完成
# ============================================
